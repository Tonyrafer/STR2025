...
void matrix_multiply(uint16_t matrix_size, uint16_t iterations)
{
    ...
    ESP_LOGI(TAG, "matrix_multiply: %d x %d elementos, %d veces",
...
void task(void *pvParameters)
{
    ...
    TimerHandle_t timer = xTimerCreate("PeriodicTimer", pdMS_TO_TICKS(config->period),
                                       pdTRUE, (void *)&sync_sem, timer_callback);
    xTimerStart(timer, 0);
    while (1)
    {
...
void app_main()
{
    ...
    task_config_t config_t1 = {"t1", 5, 1, 100};  // tarea frecuente pero rápida
    task_config_t config_t2 = {"t2", 50, 1, 500}; // tarea poco frecuente pero lenta
    // Creación de las tareas
    xTaskCreatePinnedToCore(task, "TaskA", 50 * 50, (void *)&config_t1, 1, NULL, 0); // tarea 1,core 0, prioridad 1
    xTaskCreatePinnedToCore(task, "TaskB", 50 * 50, (void *)&config_t2, 1, NULL, 0); // tarea 2,    core 0, prioridad 1
